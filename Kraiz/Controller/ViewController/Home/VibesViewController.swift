...
//    var timer : Timer?
    ...
    override func viewDidLoad() {
        ...
        if let remoteConfigKey = APPUtilites.getRemoteConfigKey() {
            print("remoteConfigKey is not nil")
            print("remoteConfigKey: \(remoteConfigKey)")

            self.usernameLabel.isHidden = true
            self.vibesSegment.isHidden = true

            // Uncomment the below line in developer mode.
//            RemoteConfig.remoteConfig().configSettings = RemoteConfigSettings.init(developerModeEnabled: true)
        ...
        let attr = NSDictionary(object: UIFont(name: "Helvetica Neue", size: 14.0)!, forKey: NSAttributedString.Key.font as NSCopying)
        ...
        // Note: This is commented as we are letting the user see unlimited public vibes.
...
//        }
        ...
        if let username = UserDefaults.standard.string(forKey: DeviceConstants.USER_NAME) {
