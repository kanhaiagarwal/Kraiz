...
typedef NS_OPTIONS(NSUInteger, OBXEntityPropertyFlag) {
    ...
    OBXEntityPropertyFlagId = 1,  /// On languages like Java, a non-primitive type is used (aka wrapper types, allowing null)
    OBXEntityPropertyFlagNonPrimitiveType = 2,  /// Unused yet
    OBXEntityPropertyFlagNotNull = 4,
    OBXEntityPropertyFlagIndexed = 8,  /// Unused yet
    OBXEntityPropertyFlagReserved = 16,  /// Unused yet: Unique index
    OBXEntityPropertyFlagUnique = 32,  /// Unused yet: Use a persisted sequence to enforce ID to rise monotonic (no ID reuse)
    OBXEntityPropertyFlagIdMonotonicSequence = 64,  /// Allow IDs to be assigned by the developer
    OBXEntityPropertyFlagIdSelfAssignable = 128,  /// Unused yet
    OBXEntityPropertyFlagIndexPartialSkipNull = 256,  /// Unused yet, used by References for 1) back-references and 2) to clear references to deleted objects (required for ID reuse)
    OBXEntityPropertyFlagIndexPartialSkipZero = 512,  /// Virtual properties may not have a dedicated field in their entity class, e.g. target IDs of to-one relations
    OBXEntityPropertyFlagVirtual = 1024,  /// Index uses a 32 bit hash instead of the value
    /// (32 bits is shorter on disk, runs well on 32 bit systems, and should be OK even with a few collisions)
    OBXEntityPropertyFlagIndexHash = 2048,  /// Index uses a 64 bit hash instead of the value
